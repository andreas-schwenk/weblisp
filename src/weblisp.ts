...
export class WebLISP {
  ...
  private eval(sexpr: SExpr, createVar = false): SExpr {
    ...
    // evaluate
    let x, y, n: number;
    let s, t, u, v, res, init, cond, body: SExpr;
    let op = "";
    let i = 0,
      j = 0;
    let b = true;
          ...
          case T.ID:
            ...
            switch (op) {
              ...
              case "DO":
                ...
                init = SExpr.nth(s, 0);
                cond = SExpr.nth(s, 1);
                body = s.cdr.type == SExprType.NIL ? null : s.cdr.cdr;
                if (
                ...
                // init
                for (t = init; t.type !== SExprType.NIL; t = t.cdr) {
                ...
                for (;;) {
                  ...
                  if (doBreak) break;
                  // body
                  for (t = body; t.type !== SExprType.NIL; t = t.cdr) {
                    this.eval(t.car);
                  }
                  // update
                  for (t = init; t.type !== SExprType.NIL; t = t.cdr) {
              ...
              case "NTH":
                ...
                switch (op) {
                  ...
                  case "REMOVE":
                    return t.remove(s);
                  case "NTH":
                    if (s.type !== SExprType.INT || (s.data as number) < 0)
                      throw new RunError(
                        "expected a non-negative integer index"
                      );
                    i = s.data as number;
                    return SExpr.nth(t, i);
                }
                break;
              // -- n arguments (n >= 0) --
              case "PROGN":
                res = SExpr.atomNIL();
                for (s = sexpr.cdr; s.type === T.CONS; s = s.cdr) {
                  res = this.eval(s.car);
                }
                return res;
              case "LIST":
