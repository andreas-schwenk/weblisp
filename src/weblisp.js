...
class WebLISP {
    ...
    eval(sexpr) {
        if(this.maxSeconds != Infinity && (Date.now()-this.startTime)/1000 > this.maxSeconds)
        ...
        if (Array.isArray(sexpr)) {
            ...
            switch (car) {
                ...
                case 'setq':
                    ...
                    if (!found)
                        this.vars[this.vars.length-1][id] = val;
                    return val;
                case 'let':
                case 'let*':
                    this.vars.push({});
                ...
                case 'if':
                    if (this.eval(cdr[0]))
                        return this.eval(cdr[1]);
                    else if (cdr.length == 3)
                        return this.eval(cdr[2]);
                    else
                ...
                default:
                    if (car in this.functions) {
                        ...
                        for (let i=0; i<fun[2].length; i++)
                            x.push(this.eval(cdr[i]));
                        y = null;
                        if (false && fun[2].length==1 && (car in this.memoization) && x[0] in this.memoization[car]) {
                        ...
                        if (fun[2].length==1) { // unary function
                            if ((car in this.memoization)==false)
                                this.memoization[car] = {};
