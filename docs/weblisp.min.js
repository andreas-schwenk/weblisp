var weblisp=(()=>{var u=(r,e)=>()=>(e||r((e={exports:{}}).exports,e),e.exports);var f=u((exports,module)=>{var WebLISP=class{constructor(){this.input="",this.sexpr=null,this.output="",this.startTime=0,this.maxSeconds=1/0,this.functions={},this.memoization={},this.vars=[{}]}getOutput(){return this.output}import(r){let e=r.split(`
`),s=[];for(let t of e){let a=t.split(";");s.push(a[0])}this.input=s.join(`
`);let l=[],h=this.input.length,d=1,v=1,i="";for(let t=0;t<h;t++){let a=this.input[t];switch(a){case"(":case")":case"+":case"*":i.length>0&&(l.push(i),i=""),l.push(a);break;case" ":case"	":case`
`:i.length>0&&(l.push(i),i="");break;default:i+=a}}i.length>0&&(l.push(i),i="");let o=["_PROG"],c=[];for(let t of l)switch(t){case"(":c.push(o),o=[];break;case")":if(c.length==0)throw Error('")" is not allowed');let a=c.pop();a.push(o),o=a;break;default:t.length>0&&t[0]>="0"&&t[0]<="9"?o.push(parseFloat(t)):o.push(t);break}this.sexpr=o}run(r=1/0){this.output="",this.functions={},this.vars=[{}],this.memoization={},this.startTime=Date.now(),this.maxSeconds=r,this.eval(this.sexpr)}eval(sexpr){if(this.maxSeconds!=1/0&&(Date.now()-this.startTime)/1e3>this.maxSeconds)throw Error("max allowed runtime exceeded!");if(Array.isArray(sexpr)){let car=sexpr[0],cdr=sexpr.slice(1),id,sum,prod,n,x,y,val,found;switch(car){case"_PROG":for(let r of cdr)this.eval(r);return null;case"car":return y=this.eval(cdr),y.length>0?y[0]:null;case"cdr":return y=this.eval(cdr),y.length>0?y.slice(1):null;case"cons":if(cdr.length!=2)throw new Error("cons requires 2 arguments");return y=this.eval(cdr),cdr;case"write":return val=this.eval(cdr[0]),console.log(val),this.output+=""+val,val;case"+":sum=0;for(let r of cdr)sum+=this.eval(r);return sum;case"*":prod=1;for(let r of cdr)prod*=this.eval(r);return prod;case"-":sum=0,n=cdr.length;for(let r=0;r<n;r++)r==1&&(sum=-sum),sum-=this.eval(cdr[r]);return sum;case"/":if(n=cdr.length,prod=this.eval(cdr[0]),n==1)return 1/prod;for(let r=1;r<n;r++)prod/=this.eval(cdr[r]);return prod;case"<":return this.eval(cdr[0])<this.eval(cdr[1]);case">":return this.eval(cdr[0])>this.eval(cdr[1]);case"terpri":return this.output+=`
